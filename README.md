...
| [cloud-identity-email-routing](cloud-identity-email-routing/) | Giving an agent its own Workspace identity and a mailbox it can receive on but never send from |
| [coreai-dynshape-poc](coreai-dynshape-poc/) | Does Apple's coreai-torch trace a zero-length dynamic-shape gather — the pattern that broke onnxruntime's CoreML EP on Style-Bert-VITS2? |
| [coremltools-dynshape-poc](coremltools-dynshape-poc/) | Same probe as coreai-dynshape-poc, run through coremltools instead — does it survive on real Neural Engine/GPU/CPU today? |
...
| [svg-rasterizer-bench](svg-rasterizer-bench/) | Rasterizing SVG from the CLI with resvg vs. sips vs. headless Chrome — what each one silently drops, and where the time goes |
...
| [swift-jscore-oop-poc](swift-jscore-oop-poc/) | Running the same JavaScriptCore plugins out-of-process so a `while(true)` can be killed — and finding the JIT hinges on an entitlement worth 12×, a passed descriptor carries operations rather than a subtree, and the bookmark that does hand over a subtree is the plain one, not the security-scoped one |
