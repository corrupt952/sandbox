...
| E12 | Does the host hold up if the helper is hostile, not just buggy? | not a rejection criterion | framing, containment and revocation all hold, **no must-fix** — a JSON depth cap is optional defense-in-depth, and flood isolation stays open |
...
## Finding 7: a hostile helper is out of scope, and that line is where it is on purpose

The threat model has been a buggy or runaway plugin, not one an attacker already controls. E12 tests the harder case anyway — a `HostileHelper` that writes raw bytes under the frame protocol and plays every probe from an attacker's side — so the line is drawn on measurement rather than assumption. It holds, at four points, with nothing that must be fixed.

Framing takes every malformed input a cooperating helper could never send. A length prefix claiming 4 GiB and one a byte over the 32 MiB cap are both refused as oversize; invalid UTF-8 and a valid-JSON-but-not-an-object body are refused as malformed; a frame that announces N bytes and sends N−10 and holds the socket open is stopped by a receive deadline rather than hanging. A valid 32 MiB frame is accepted and parsed, growing the host by ~144 MiB — bounded by the cap, but 4.5× the frame, so N such helpers cost N×144 MiB. Nothing crashed. The one number with an edge is nesting: `JSONSerialization` accepts arrays 512 deep and rejects 513 by throwing, not crashing, so a plugin returning past 512 hits a clean refusal. A host-side depth cap would make that refusal earlier and predictable, but it prevents no crash — it is optional defense-in-depth, not a fix.

Revocation belongs to the host by construction. The helper holds no capability, only asks for one, so a host that adopts a revoking policy — shown with an experiment-local handler — cuts a hijacked helper off mid-session, and the helper has no state to work around it. That is the E7/E8 result (the policy gate lives in the host) read under an attacker. It also sharpens the capability-granularity choice: handed one plain bookmark, a hostile helper walks the whole subtree with no further host involvement; on per-file reverse RPC, it gets only what the host allows each call and the host can refuse the rest. Under a hijack the per-file RPC the design already keeps is the tighter of the two.

Containment has to be tested on a live grant to mean anything, and this is where the first attempt was wrong. `openat` through a passed descriptor is denied under App Sandbox for any target — E7 — so a denied canary there proves only that the descriptor granted nothing to begin with, not that an escape was blocked. The real test is the plain bookmark, where the grant is live: the helper opens the files it was granted, and every escape from there — `..`, an absolute path to the canary, a symlink pointing out of the subtree — is denied. A live grant does not widen for an attacker any more than for a cooperating helper.

What E12 does not close is one flooding question. A helper firing 50,000 reverse RPCs buffers harmlessly, the host's memory stays bounded, and a cooperating helper's ticks are undisturbed — but this host services helpers on one thread in sequence, so that last part is not isolation the run demonstrated, only an arrangement that did not collide. True isolation would follow from a per-helper servicing thread the host does not yet run, and a flooder that never reads its replies can still block a synchronous servicing send once the socket fills. That is bounded, not resolved, and it is the one item this leaves open.

So the line stays where it was: a hostile helper is out of scope. But it is out of scope having been measured — framing, revocation and containment hold, the flood is bounded, and the only code change on the table is an optional depth cap.
...
What a reboot does to that cost was checked separately, and the answer splits. Run just after a restart, against the same already-built binary and without rebuilding, E1's first spawn came back at 6.10 ms, not the 294 ms of a session's first spawn, and E1b's unvalidated-copy cost was unchanged at 100 ms p50 against 99.69 before the reboot. So the validation cache a validated binary relies on survives a reboot — the unvalidated cost is not a daily first-launch tax, only an install-or-update one — and the unvalidated cost itself is invariant across reboots, which fits E10's finding that validation is keyed to the file. What this does not isolate is the 294 ms itself: that is dyld and JavaScriptCore faulting in cold, and by the time this ran the machine was warm (load ~2, other processes having touched JSC), so a genuinely cold boot with JSC never yet used is still unmeasured. The cache-survives-a-reboot conclusion rests on the 6 ms first spawn and the flat E1b, both solid; the cold-JSC first-fault cost on a bare boot is not.
...
A hostile helper is out of scope, now by measurement (Finding 7) rather than by assertion. E12 found the host holds at framing, revocation and containment with no must-fix, but it left one thing open and one thing untested. Open: flood isolation, which this single-threaded host arranges away rather than enforces, and a reply-ignoring flooder that can block a synchronous servicing send — both need a per-helper servicing thread the host does not run. Untested: E12's revocation is an experiment-local handler, not a shipping feature, and its depth-cap suggestion is unbuilt. The `HostileHelper` itself runs unsandboxed, so E12a/b are host-side only; E12d/e borrow the existing sandboxed helper.
